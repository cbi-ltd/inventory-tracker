...
public class PaymentService {
        ...
        Specification<Payment> specification = PaymentSpecification.filter(request)
                    .and((root, query, criteriaBuilder) -> criteriaBuilder.equal(
                                    root.get("sale").get("station")
                                            .get("merchant").get("id"), merchant.getId()));
    ...
    public List<PaymentResponse> getAllPayments() {
        ...
        return paymentRepository.findAllByMerchantIdOrderByPaymentTimeDesc( merchant.getId()).stream()
                                    .map(paymentMapper::toResponse).toList();
    }
    ...
    public List<PaymentResponse> getPaymentsByStatus(PaymentStatus status) {
        ...
        return paymentRepository.findBySale_Station_Merchant_IdAndPaymentStatusOrderByPaymentTimeDesc(merchant.getId(), status)
                                    .stream().map(paymentMapper::toResponse).toList();
    }
    ...
    public List<PaymentResponse> getPaymentsByMethod(PaymentMethod method) {
        ...
        return paymentRepository.findBySale_Station_Merchant_IdAndPaymentMethodOrderByPaymentTimeDesc(merchant.getId(), method)
                                    .stream().map(paymentMapper::toResponse).toList();
    }
    ...
    private void updateCurrentPumpAudit(Sale sale) {
        ...
        if (saleStation.getMerchant() == null || !saleStation.getMerchant().getId().equals(merchant.getId())) {
        ...
        if (assignment.getStation().getMerchant() == null || !assignment.getStation().getMerchant().getId().equals(merchant.getId())) {
            throw new ResourceNotFoundException("Pump assignment not found");
        }
    ...
    private void verifySaleOwnership(Sale sale) {
            ...
            if (sale.getStation() == null || sale.getStation().getMerchant() == null ||
                    !sale.getStation().getMerchant().getId().equals(merchant.getId())) {
    ...
    private void verifyStationOwnership(Station station) {
        ...
        if (station == null || station.getMerchant() == null ||
                    !station.getMerchant().getId().equals(merchant.getId())) {
